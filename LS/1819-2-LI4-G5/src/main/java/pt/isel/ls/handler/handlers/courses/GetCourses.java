package pt.isel.ls.handler.handlers.courses;

import java.util.HashMap;
import java.util.LinkedList;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import pt.isel.ls.Queries;
import pt.isel.ls.handler.GetHandlerBase;
import pt.isel.ls.result.Result;
import pt.isel.ls.result.courses.GetCoursesResult;
import pt.isel.ls.template.Template;
import pt.isel.ls.transactionmanager.TransactionManager;


public class GetCourses extends GetHandlerBase {

    private static final Logger logger = LoggerFactory.getLogger(GetCourses.class);
    private TransactionManager<Result> transactionManager;

    public GetCourses(TransactionManager<Result> transactionManager) {
        this.transactionManager = transactionManager;
    }

    @Override
    public Result executeCommand(HashMap<String, LinkedList<String>> values) {
        return transactionManager.run(connection ->
                new GetCoursesResult(Queries.getCourses(values, connection))
        );
    }

    @Override
    public Template getTemplate() {
        return Template.of("/courses");
    }

    @Override
    public String description() {
        return "shows all courses";
    }
}
